    ...
    with get_conn() as conn:
        conn.execute(
            ...
            INSERT INTO sessions (
                id, resident_id, preferred_name, locale, speech_register,
...
def append_turn(session_id: str, role: str, text: str) -> dict[str, Any] | None:
    session = get_session(session_id)
    if not session or session["status"] != "active":
        return None
    transcript = session["transcript"]
    transcript.append({"role": role, "text": text, "at": _utc_now()})
    with get_conn() as conn:
        conn.execute(
...
def _row_to_session(row: sqlite3.Row) -> dict[str, Any]:
    return {
        ...
        "locale": row["locale"],
        "speech_register": row["speech_register"],
        "room_id": row["room_id"],
        "status": row["status"],
